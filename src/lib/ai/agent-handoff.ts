...
export async function routeToAgent(
  ...
  // Execute handoff if the lead has moved to a different stage. Skipped in
  // preview/dry-run (task delegation): the handoff is a conversation-state write,
  // and a preview must not mutate anything before the human hits Send. The draft
  // is still generated by whichever agent `routing.agent` selects below.
  if (routing.needsHandoff && routing.reason && !context.preview) {
