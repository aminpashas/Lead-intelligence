...
export function formatEscalationTime(iso: string): string {
...
export function buildCheckinSmsBody(params: {
  ...
  return `Hi ${params.firstName || 'there'}, quick check-in from ${params.practiceName} — will we see you at ${formatEscalationTime(params.scheduledAt)} today? Reply YES to confirm, or reply here if you need to reschedule.`
}
  ...
  for (const apt of (tier2 || []) as unknown as EscalationAppointment[]) {
    ...
    try {
      ...
      await supabase.from('lead_activities').insert({
        ...
        title: `High no-show risk (${apt.no_show_risk_score}) — personal call recommended before ${formatEscalationTime(apt.scheduled_at)}`,
        metadata: {
          automated: true,
      ...
      await dispatchConnectorEvent(
        supabase,
        {
          ...
          data: {
            ...
            metadata: {
              appointment_time: formatEscalationTime(apt.scheduled_at),
