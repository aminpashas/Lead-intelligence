...
export function LeadsTable({
  ...
  /**
   * One filter dropdown. Base UI's <Select.Value> renders the raw current
   * value, which showed a bare "all" on every idle filter. We feed it an
   * `items` map (value → trigger text) instead. The category name is always
   * shown so you can tell what a filter does at a glance: idle reads "Source" /
