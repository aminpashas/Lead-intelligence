...
function ScoreCell({ label, value, accent }: { label: string; value: number; accent?: string }) {
  const pct = Math.max(0, Math.min(100, (Number(value) || 0) * 10))
  return (
    <div className="rounded-lg border border-aurea-border bg-aurea-canvas px-3 py-2">
      <div className="flex items-baseline justify-between gap-2">
        <span className="text-[11px] text-aurea-ink-3">{label}</span>
        <span className={`aurea-display text-[17px] tabular-nums ${accent || 'text-aurea-ink'}`}>
          {value}<span className="text-[11px] text-aurea-ink-3">/10</span>
        </span>
      </div>
      <div className="mt-1.5 h-[3px] overflow-hidden rounded-full bg-aurea-surface-2">
        <div className="h-full rounded-full bg-aurea-ink-2/70" style={{ width: `${pct}%` }} />
      </div>
    </div>
  )
}
...
function FactCell({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-lg border border-aurea-border bg-aurea-canvas px-3 py-2">
      <div className="text-[11px] text-aurea-ink-3">{label}</div>
...
export function InsightsPanel({ analysisResult, followUpResult }: { analysisResult: any; followUpResult: any }) {
  ...
  return (
    <div className="max-h-96 overflow-y-auto border-b border-aurea-border bg-aurea-surface">
        ...
        {ca && (
          <section className="space-y-2.5">
            ...
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
              <ScoreCell label="Engagement" value={ca.engagement_score} accent="text-aurea-primary" />
              <ScoreCell label="Trust" value={ca.trust_score} accent="text-aurea-primary" />
              <ScoreCell label="Emotion" value={ca.emotional_score} />
              <ScoreCell label="Urgency" value={ca.urgency_score} />
              <ScoreCell label="Sales Pressure" value={ca.sales_pressure_level} />
              <ScoreCell label="Empathy" value={ca.empathy_level} />
              <ScoreCell label="Rapport" value={ca.rapport_building_score} />
              {ca.compliance_score != null && (
                <div className="rounded-lg border border-aurea-border bg-aurea-canvas px-3 py-2">
                  <div className="flex items-baseline justify-between gap-2">
                    <span className="flex items-center gap-1 text-[11px] text-aurea-ink-3">
                      <Shield className="h-3 w-3" strokeWidth={1.75} /> HIPAA
                    </span>
                    <span className={`aurea-display text-[17px] tabular-nums ${ca.compliance_score < 90 ? 'text-aurea-amber' : 'text-aurea-ink'}`}>
                      {ca.compliance_score}<span className="text-[11px] text-aurea-ink-3">%</span>
                    </span>
                  </div>
                  <div className="mt-1.5">
                    {ca.compliance_issues?.length > 0 ? (
                      <span className="inline-flex items-center gap-1 text-[10px] font-medium text-aurea-rose">
                        <AlertTriangle className="h-2.5 w-2.5" strokeWidth={1.75} />
                        {ca.compliance_issues.length} issue{ca.compliance_issues.length > 1 ? 's' : ''}
                      </span>
                    ) : (
                      <span className="text-[10px] text-aurea-ink-3">No issues</span>
                    )}
                  </div>
                </div>
              )}
            ...
            {ca.coaching_notes && (
              <div className="border-l-2 border-aurea-gold py-1 pl-3.5">
                ...
                <p className="text-[12.5px] leading-relaxed text-aurea-ink-2">{ca.coaching_notes}</p>
              </div>
            )}
          </section>
        )}
        ...
        {pp && (
          <section className="space-y-2.5">
            ...
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
              ...
              <ScoreCell label="Anxiety" value={pp.anxiety_level} />
              <ScoreCell label="Motivation" value={pp.motivation_level} />
            </div>
            {pp.ai_summary && (
