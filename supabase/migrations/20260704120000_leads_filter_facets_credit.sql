-- Extend leads_filter_facets with a credit_ranges facet so the Leads view can
-- offer a credit filter. SECURITY INVOKER (default) — runs under the caller's
-- RLS, scoped by p_org. Excludes null/'unknown' so the dropdown only surfaces
-- real buckets (today the imported book has zero credit data, so this returns
-- [] and the dropdown stays hidden client-side).
...
as $$
  select jsonb_build_object(
    ...
    'credit_ranges', coalesce((
      select jsonb_agg(jsonb_build_object('value', v, 'count', n) order by n desc)
      from (
        select credit_range as v, count(*) as n
        from leads
        where organization_id = p_org
          and credit_range is not null and credit_range <> 'unknown'
        group by 1
        order by count(*) desc
      ) cr
    ), '[]'::jsonb)
