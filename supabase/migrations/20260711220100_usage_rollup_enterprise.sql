-- Migration: usage_rollup gains enterprise filtering (p_enterprise)
--
-- Lets the agency console roll spend up for a whole enterprise (DSO) — per-location
-- rows PLUS an enterprise total (summed in the app layer). Grouping stays per-org so
-- the existing `byOrg` shape in src/lib/billing/usage-live.ts is unchanged.
--
-- Replay-drift guard (see project memory "migration-replay-drift"): usage_rollup has
-- already been redefined once (2-arg -> 3-arg with p_until). Adding a 4th arg would
-- leave the 3-arg overload resolvable, making calls ambiguous. Drop the older
-- overloads first so exactly one signature exists.

drop function if exists public.usage_rollup(timestamptz, uuid);
drop function if exists public.usage_rollup(timestamptz, uuid, timestamptz);
