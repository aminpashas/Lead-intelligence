-- Extend the junk pass of reclassify_off_funnel_contacts with a business/insurer/
-- other-practice name signal (vendor & insurer calls: "Delta Dental", "Kaiser
-- Permanente", "Parkside Dental", "Aetna", "… Pharmacy", "… LLC"). A person is
-- never named these. Whole-word (\y) match so a real surname is never clipped.
-- Mirrors BUSINESS_KEYWORDS in lib/leads/junk-contact.ts. Prod sample: all orgs.
...
declare
  ...
  biz_re text := '\y(dental|dentist|dentistry|orthodont\w*|endodont\w*|periodont\w*|prosthodont\w*|invisalign|pharmacy|clinic|hospital|medical|medicaid|medicare|insurance|kaiser permanen\w*|aetna|cigna|metlife|humana|anthem|delta ?dental|unitedhealth\w*|healthplan|health plan|llc|inc)\y';
